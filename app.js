...
const AnalyticsCards = () => {
    ...
    const months = Math.max(1, Math.round(
        (Date.now() - Math.min(...submissions.map(x => new Date(x.created_at)))) / (30 * 24 * 3600 * 1000)));
    ...
    return ul(items.map(item =>
        li([
            ...
            small([
                ...
                ' | ', i('.fas.fa-user'), ' ',
                a(
                    { props: { href: `https://news.ycombinator.com/user?id=${item.author}`, target: '_blank' } },
                    item.author,
                ),
...
const onAnalyze = async () => {
    ...
    try {
        ...
        state.submissions = allHits.filter(h => normalizeDomain(h.url) === normalized)
    } catch (err) {
        console.error(err)
